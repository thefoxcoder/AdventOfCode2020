...
function getFirstLetterPosition(input) {
    var regex = /([0-9]+)-/
    return parseInt(regex.exec(input)[1]) - 1;
}

function getSecondLetterPosition(input) {
    var regex = /[0-9]+-([0-9]+)/
    return parseInt(regex.exec(input)[1]) - 1;
}

function getPassword(input) {
    const passwordRegex = /:[\s]*([A-Za-z]+)/;
    return passwordRegex.exec(input)[1];
}

function getRequiredLetter(input) {
    var regex = /([A-Za-z]):/
    return regex.exec(input)[1]
}

function isInputValid(input) {
    var firstPos = getFirstLetterPosition(input);
    var secondPos = getSecondLetterPosition(input);
    var password = getPassword(input);
    var requiredLetter = getRequiredLetter(input);
    
    if(password[firstPos] === requiredLetter && password[secondPos] === requiredLetter){
        return false
    }

    return password[firstPos] === requiredLetter || password[secondPos] === requiredLetter
}
